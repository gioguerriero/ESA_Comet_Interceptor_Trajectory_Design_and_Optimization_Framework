...
- **Design objective.** For a given target, the primary output is the **ΔV–ToF Pareto front**; escape velocity is an *outcome*, not an objective.
...
This section describes the purpose, inputs, and outputs of each phase. Mathematical formulations and per-function details are intentionally left to the future dedicated READMEs.
...
Every `.m` file begins with a header documenting its purpose, inputs, and outputs. Detailed, component-level READMEs (for the nonlinear constraints and individual optimization phases) are planned as follow-ups to this document.
...
*This document is the primary entry point for understanding the framework. It describes the workflow and architecture; the mathematical formulations and per-function contracts belong to the source-file headers and to the forthcoming component-level READMEs.*
